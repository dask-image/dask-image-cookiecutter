...
Go to your GitHub account and create a new repo named ``mypackage``, where ``mypackage`` matches the ``[project_import]`` from your answers to running cookiecutter. This is so that services can find it when we get to Step 5.

``If your virtualenv folder is within your project folder, be sure to add the virtualenv folder name to your .gitignore file.``

You will find one folder named after the ``[project_import]``. Move into this folder, and then setup git to use your GitHub repo and upload the code:
